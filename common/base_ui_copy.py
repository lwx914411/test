...
from common.operation_error import OperationError
...
class BaseUI():
    ...
    def find_element(self, locator):
        ...
        if not isinstance(locator, tuple):
            ...
            try:
                element = self.according_wait(EC.visibility_of_element_located(locator))
                if element.is_displayed():
                    return element
            except:
                self.log.error(f'{self.driver.current_url}页面中未能找到元素"{locator}"')
    ...
    @OperationError.selenium_error
    def is_switch_frame(self, locator):
        """判断该frame是否可以switch进去，如果可以的话，返回True并且switch进去，否则返回False"""
        return self.according_wait(EC.frame_to_be_available_and_switch_to_it(locator))
        # try:
        #     return self.according_wait(EC.frame_to_be_available_and_switch_to_it(locator))
        # except TimeoutException:
        #     self.log.error('%s元素没有定位到' % str(locator))
...
if __name__ == '__main__':
    b = BaseUI()
    # b.find_element((By.ID,'kw')).send_keys('测试')
    # b.click((By.XPATH, '//*[@id="s-top-left"]/a'))
    # b.send_keys((By.ID,'kw'),'测试')
    # print(b.find_element((By.ID,'kw')).size)
    # b.move_to_element((By.XPATH, '//*[@id="s-top-left"]/a'))
    # b.switch_window_handle(r'https://www.baidu.com/')
    print(b.is_visibility(By.ID,'kw'))
